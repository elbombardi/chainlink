...
:nonce, :from_address, :to_address, :encoded_payload, :value, :gas_limit, :error, :broadcast_at, NOW(), :state, :meta, :subject, :pipeline_task_run_id, :min_confirmations, :evm_chain_id, :access_list, :simulate
) RETURNING *`
...
func (o *orm) FindEthTxWithAttempts(etxID int64) (etx EthTx, err error) {
	err = postgres.NewQ(o.db).Transaction(o.logger, func(q postgres.Queryer) error {
		if err := q.Get(&etx, `SELECT * FROM eth_txes WHERE id = $1`, etxID); err != nil {
